...
const UI = {
  ...
  renderDashboard() {
    ...
    let outstandingTotal = 0; let dueSoonList = []; let overdueList = [];
    ...
    if (activeRentals.length > 0) {
      ...
      activeRentals.forEach(r => {
        ...
        html += `<div class="rental-card ${statusClass}">
          ...
          <div class="rental-row"><span class="rental-label">${st.isOverdue ? 'Due Date' : 'Next Due'}</span><span class="rental-value">${st.isOverdue ? `<span style="color:var(--danger)">${fmtDate(st.nextDueDate.toLocaleDateString('en-CA'))}</span> <span class="badge badge-danger">${st.daysOverdue}d overdue</span>` : st.isDueSoon ? `${fmtDate(st.nextDueDate.toLocaleDateString('en-CA'))} <span class="badge badge-warning">${st.daysUntilDue === 0 ? 'Due today' : st.daysUntilDue === 1 ? 'Due tomorrow' : `In ${st.daysUntilDue}d`}</span>` : fmtDate(st.nextDueDate.toLocaleDateString('en-CA'))}</span></div>
